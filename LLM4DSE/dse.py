...
# 转换字符串类型的 model_type 为 ModelType 枚举（延迟导入避免 torch_geometric 初始化问题）
def _convert_model_type_if_needed(model_type_value):
    """如果 model_type 是字符串，转换为 ModelType 枚举"""
...
#gumbel_args参数
gumbel_args = GumbelArgs(learn_temp=FLAGS.learn_temp, temp_model_type=temp_model_type, tau0=FLAGS.tau0,
                                 temp=FLAGS.temp, gin_mlp_func=gin_mlp_func)
#环境network参数
env_args = \
EnvArgs(model_type=env_model_type, num_layers=FLAGS.env_num_layers, env_dim=FLAGS.env_dim,
...
#行为network参数
action_args = \
        ActionNetArgs(model_type=act_model_type, num_layers=FLAGS.act_num_layers,
...
class GNNModel():
    def __init__(self, path, saver, multi_target=True, task='regression', num_layers=FLAGS.num_layers, D=FLAGS.D,
        ...
        """
        #这里的">>>"是用来提醒用户处于交互式的代码运行环境，是可以输入代码进行调试的。
        >>> self.encoder.keys()
        ...
        self.model = Net().to(
            FLAGS.device)
        # 获得训练好的参数
        self.model.load_state_dict(torch.load(join(self.model_path), map_location=torch.device('cuda:0')))
        saver.info(f'loaded {self.model_path}')
        self.encoder = load(self.encoder_path)

    # 编码结点，将path_graph数据转换为可输入模型中的数据
    ...
    # 将config中的TARGETS的perf和quality作出了解释，当然这里quality是指perf的
    ...
    def eff_as_quality(self, new_result: Result) -> float:
        ...
        """
        area = sum([0.25 * ceil(max(0.0, abs(u))) for k, u in new_result.res_util.items() if k.startswith('util')])
        return 1 / (new_result.perf * area)

    # 开始测试
    def test(self, loader, code_loader, config, mode='regression'):
        self.model.eval()
        ...
        for data in loader:
            ...
            inx += 1
            out_dict, loss, loss_dict = self.model(data, code_emb)
            # 如果是回归
            if mode == 'regression':
                for i in range(len(out_dict['perf'])):
                    ...
                    for target_name in target_list:
                        ...
                        # 将这些数值存放至curr_result中
                        if target_name == 'perf':
                            curr_result.perf = pow(100, out_value) - 1
                            # 判断是否经过取对数的操作,避免有负数的产生，默认值为False
                            if FLAGS.encode_log:
                            ...
                            if out_value > 0:
                                tmp = pow(100, out_value) - 1
                                curr_result.res_util[target_name] = tmp
        ...
        # --- Causal matrix quick summary (Top-K) ---
        # 仅用于验证因果模块是否在 DSE 推理链路中工作
        try:
            if getattr(FLAGS, "use_causal", False) and hasattr(self.model, "_last_alpha_matrix"):
                ...
                pragma_ids = getattr(self.model, "_last_pragma_ids", None)
                if alpha is not None and alpha.numel() > 0:
                    # alpha: [batch, num_pragmas, num_targets]
                    score = alpha.abs().mean(dim=(0, 2))  # [num_pragmas]
                    ...
                    for vv, ii in zip(topv.tolist(), topi.tolist()):
                        pid = pragma_ids[ii] if isinstance(pragma_ids, list) and ii < len(pragma_ids) else str(ii)
...
class Explorer():
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True, prune_invalid=False):
        ...
        self.codebert = AutoModelForMaskedLM.from_pretrained("/home/test/xulei/passion/codebert").to(FLAGS.device)
        self.tokenizer = AutoTokenizer.from_pretrained("/home/test/xulei/passion/codebert")
        # 检查非法配置
    ...
    def update_best(self, result: Result):
        ...
        # norm_method默认为speedup_log2
        update_flag = False
        if 'speedup' in FLAGS.norm_method:
            REF = min
        else:
            REF = max
        # 得到当前key_perf_dict中最大值的key和values
        ...
        # 如果点有效且比当前最大值还大也不在key_perf_dict中，对一些信息进行更新
    ...
    def get_results(self, population: List[DesignPoint]) -> List[Result]:
        ...
        for point in population:
            code_list.append(self.gen_code_embedding(point, self.kernel_name))
        test_loader = DataLoader(data_list, batch_size=self.batch_size)  # TODO
        code_loader = code_list
        results = self.GNNmodel.test(test_loader, code_loader, self.config['evaluate'], mode='regression')
        return results
...
class LMEAExplorer(Explorer):
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True,
        ...
        if self.run_dse:
            ...
            i = 1
            with open(join(get_root_path(), f'best_result/LMEA/{FLAGS.llm_model}', f'{kernel_name}.pickle'), 'wb') as handle:
...
class LMSAExplorer(Explorer):
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True,
        ...
        if self.run_dse:
            ...
            self.log.info('Best Results Found:')
            i = 1
            with open(join(get_root_path(), f'best_result/LMSA/{FLAGS.llm_model}', f'{kernel_name}.pickle'), 'wb') as handle:
                pickle.dump(self.best_save_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
                handle.flush()
            # 输出最好结果
            for _, result in sorted(self.best_results_dict.items()):
...
class LMACOExplorer(Explorer):
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True,
        ...
        if self.run_dse:
            ...
            self.log.info('Best Results Found:')
            i = 1
            with open(join(get_root_path(), f'best_result/LMACO/{FLAGS.llm_model}', f'{kernel_name}.pickle'), 'wb') as handle:
                pickle.dump(self.best_save_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
                handle.flush()
            # 输出最好结果
            for _, result in sorted(self.best_results_dict.items()):
    ...
    class Ant:
        def __init__(self, aco):
            self.aco = aco
            self.solution = {}  # 存储参数选择，例如 {"cores":4, "freq":2.0}
            self.fitness = 0
            self.violation = 0  # 约束违反度
    ...
    def calculate_probability(self, param):
        ...
        if type(value[0]) == str:
            ...
            values = value
        heuristic = np.array([1.0 / (v + 1e-6) for v in values])  # 示例启发式（可替换）
...
class EAExplorer(Explorer):
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True,
        ...
        if self.run_dse:
            ...
            self.log.info('Best Results Found:')
            i = 1
            with open(join(get_root_path(), f'best_result/EA', f'{kernel_name}.pickle'), 'wb') as handle:
                pickle.dump(self.best_save_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
                handle.flush()
            # 输出最好结果
            for _, result in sorted(self.best_results_dict.items()):
...
class SAExplorer(Explorer):
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True,
        ...
        if self.run_dse:
            ...
            self.log.info('Best Results Found:')
            i = 1
            with open(join(get_root_path(), f'best_result/SA', f'{kernel_name}.pickle'), 'wb') as handle:
                pickle.dump(self.best_save_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
                handle.flush()
            # 输出最好结果
            for _, result in sorted(self.best_results_dict.items()):
    ...
    def run(self) -> None:
        ...
        while self.explored_point <= self.stop_cond or temperature_1 >= FLAGS.stop_temperature:
            # evaluate solution
            print('------------------------------------------------------')
            print(f'explored point {self.explored_point}/{self.stop_cond}')
            print('------------------------------------------------------')
...
class ACOExplorer(Explorer):
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True,
        ...
        if self.run_dse:
            ...
            self.log.info('Best Results Found:')
            i = 1
            with open(join(get_root_path(), f'best_result/ACO', f'{kernel_name}.pickle'), 'wb') as handle:
                pickle.dump(self.best_save_results, handle, protocol=pickle.HIGHEST_PROTOCOL)
                handle.flush()
            # 输出最好结果
            for _, result in sorted(self.best_results_dict.items()):
...
class ExhaustiveExplorer(Explorer):
    def __init__(self, path_kernel: str, kernel_name: str, path_graph: str, run_dse: bool = True,
        ...
        if self.run_dse:
            ...
            self.log.info('Best Results Found:')
            i = 1
            with open(join(get_root_path(), f'best_result/ref', f'{kernel_name}.pickle'), 'wb') as handle:
                # 将best_results_dict序列化的写入handle中，并立即保存
