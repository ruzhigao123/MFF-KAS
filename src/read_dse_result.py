...
dse_KERNELS = ['stencil', 'nw', 'gesummv', 'heat-3d', 'jacobi-1d', 'jacobi-2d', 'seidel-2d']
if __name__ == '__main__':
    ...
    for kernel in dse_KERNELS:
        with open(osp.join(get_root_path(), f'best_result/ref/{kernel}.pickle'), 'rb') as f:
            data = pickle.load(f)
            data_list = []
            for i in data:
                data_1, _ = i
    ...
    av1 = 0
    for kernel in dse_KERNELS:
        with open(osp.join(get_root_path(), f'best_result/LMACO/deepseek-r1/{kernel}.pickle'), 'rb') as f:
            data = pickle.load(f)
            tmp = []
